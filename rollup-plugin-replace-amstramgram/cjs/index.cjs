...
/**
 * @typedef ReplaceOption = [RegExp, String]
 */
...
const
  ...
  //DISPLAY RED WARNING
  warn = error => console.log(red(bold(`-- AMSTRAMGRAM-REPLACE-ERROR --\n|  ${error}`))),
  //DISPLAY MAGENTA NOTIFICATION
  notify = notification => console.log(magenta(bold(`-- AMSTRAMGRAM-REPLACE-NOTIFICATION --\n|  ${notification}`))),
  //DISPLAY GREEN MESSAGE
  inform = msg => console.log(green(bold(`-- AMSTRAMGRAM-REPLACE-PLUGIN --\n|  ${msg}`)));
 ...
 */
function getProcessedFilePath(name, ext, path, rename) {
...
function getProcessedFilesPaths(src, dest, root, rename) {
  ...
  dest.forEach(d => {
    ...
    processedFilesPath.push(path.join(destinationFolder, rename ? getProcessedFilePath(name, ext, src, rename) : name + ext ));
  });
 ...
 *    @property {ReplaceOption | Array<ReplaceOption>} replace (required) : a ReplaceOption or an array of ReplaceOption.
 *    @property {String} processHook: defines the hook used to process the files (default : 'buildStart').
...
*/
function rollupPluginReplaceAmstramgram(options) {
  let {
    jobs,
    replace = [],
    watch = false,
  ...
  if (!Array.isArray(replace)) {
    if (warnOnError) warn(`${italic(`"replace"`)} option must be an array.\n|  I received ${yellow(stringify(replace))}`);
    return
  } else if (replace.length == 0) {
    if (warnOnError) warn(`${italic(`"replace"`)} option is empty.\n|  I've nothing to do !`);
    return
  } else if (replace.length == 2 && !Array.isArray(replace[0]) && !Array.isArray(replace[1])) {
    if (!(replace[0] instanceof RegExp)) {
      if (warnOnError) warn(`First item of ${italic(`"replace"`)} option must be a regular expression.\n|  I received ${yellow(stringify(replace[0]))}`);
      return
    } else if (typeof replace[1] !== 'string') {
      if (warnOnError) warn(`Second item of ${italic(`"replace"`)} option must be a string.\n|  I received ${yellow(stringify(replace[1]))}`);
      return
    }
    replace = [replace];
  } else if (replace.some(item => !Array.isArray(item))) {
    if (warnOnError) warn(`Each item of ${italic(`"replace"`)} option must be an array.\n|  I received ${yellow(stringify(replace))}`);
    return
  } else if (replace.some(item => item.length != 2)) {
    if (warnOnError) warn(`Each item of ${italic(`"replace"`)} option must be an array of two elements.\n|  I received ${yellow(stringify(replace))}`);
    return
  } else if (replace.some(item => !(item[0] instanceof RegExp))) {
    if (warnOnError) warn(`Each of the first item of one of the elements of ${italic(`"replace"`)} option must be a regular expression.\n|  I received ${yellow(stringify(replace))}`);
    return
  } else if (replace.some(item => typeof item[1] !== 'string')) {
    if (warnOnError) warn(`Each of the second item of one of the elements of ${italic(`"replace"`)} option must be a regular expression.\n|  I received ${yellow(stringify(replace))}`);
    return
  }
  replace = new Map(replace);
  ...
  if (processHook == 'writeBundle') processHook = '';
  ...
  async function process(rollup) {
    ...
    if (filesToProcess.filter(file => file.toUpdate).length == 0) return
    await Promise.all(filesToProcess.filter(file => file.toUpdate).map(async (file) => {
      //Read file content
      let fileContent = await fs.promises.readFile(file.src, "utf-8").catch(error => {
        if (warnOnError) warn(`unable to read ${italic(file.src)} file.\n|  ${error}`);
        processError = true;
      });

      if (!processError) {
        replace.forEach((string, regexp) => fileContent = fileContent.replace(regexp, string));
        ...
        await Promise.all(file.dest.map(async (destFile) => {
          ...
          //Write the file
          fs.writeFile(destFile, fileContent, error => {
            if (error) {
  ...
  return {
    ...
    async [watchChangeHook](id, change) {
      ...
      if (needToUpdate && (change.event == 'create' || change.event == 'delete')) {
        ...
        filesToProcess.forEach(file => {
          if (path.resolve(file.src) == path.resolve(id)) file.toUpdate = true;
        });
      }
    },
    ...
    async [processHook]() {
      ...
      //If the replace process has already been completed, we simply return
      if (done) return
      await process(this);
      done = true;
    },
...
module.exports = rollupPluginReplaceAmstramgram;
